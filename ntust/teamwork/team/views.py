    ...
    if request.method == "POST":
        if 'login' in request.POST:  # 一個表單兩個action時要用到 此為按下登入時觸發的action
            ...
            if len(user) != 0:
                ...
                userid = user.id
                user_has_activity = UserHasActivity.objects.filter(
                    user_id=userid)  # 擷取出此user有的activity
...
def questionary(request, questionary_type_id=None):
    ...
    if questionary_type_id is not None and int(questionary_type_id) in (1, 2, 3):
        ...
        qn_list = Question.objects.filter(
            questionary_type_id=questionary_type_id)
    else:
    ...
    # 第三個變數代表會傳給第二個變數規定的html的變數有哪些
    return render(request, 'team/questionary.html', locals())
...
def new(request):
    ...
    if request.method == 'POST':
        ...
        new_pia = Activity.objects.create(name=name, pia_manager_name=piam_name, pia_manager_email=piam_email,
                                          activity_manager_name=activity_manager_name, activity_manager_email=activity_manager_email, description=description)
...
def dataflow_saveLane(request):
    ...
    saveLane = request.POST.get('postdata')
    saveLane = json.loads(saveLane)

    swimlane_object_post.swimlane_json = json.loads(saveLane['swimlane'])
    swimlane_object_post.svg = saveLane['svg']
...
def addNode(saveTemp, pk):
    ...
    if saveTemp["asset_type"] == "Lane1":
        ...
        participant = Participant.objects.get(
            activity_id=pk, name=saveTemp["name"])
        ...
        if len(ProcessHasParticipant.objects.filter(process_id=process.id, participant_id=participant.id)) == 0:
            ProcessHasParticipant.objects.create(
                process_id=process.id, participant_id=participant.id)
        ...
        if len(ProcessHasSystem.objects.filter(process_id=process.id, system_id=system.id)) == 0:
            ProcessHasSystem.objects.create(
                process_id=process.id, system_id=system.id)
...
def removeNode(saveTemp, pk):
    if saveTemp["asset_type"] == "Lane1":
        participant = Participant.objects.get(
            activity_id=pk, name=saveTemp["name"])
        process = Process.objects.get(activity_id=pk, name=saveTemp["process"])
        itemDelete = ProcessHasParticipant.objects.get(
            process_id=process.id, participant_id=participant.id)
        itemDelete.delete()
        ...
        system = System.objects.get(activity_id=pk, name=saveTemp["name"])
        process = Process.objects.get(activity_id=pk, name=saveTemp["process"])
        itemDelete = ProcessHasSystem.objects.get(
            process_id=process.id, system_id=system.id)
        itemDelete.delete()
        ...
        pii = Pii.objects.get(activity_id=pk, name=saveTemp["name"])
        process = Process.objects.get(activity_id=pk, name=saveTemp["process"])
        itemDelete = ProcessHasPii.objects.get(
            process_id=process.id, pii_id=pii.id)
        itemDelete.delete()
...
def renameNode(saveTemp, pk):
    ...
    if saveTemp["asset_type"] == "Lane1":
        process = Process.objects.get(activity_id=pk, name=saveTemp['process'])
        participant_old = Participant.objects.get(
            activity_id=pk, name=saveTemp['old_name'])
        if len(ProcessHasParticipant.objects.filter(participant_id=participant_old.id)) == 1:  # 舊的name只有一個物件
            ...
            # 新的name已有物件
            elif len(Participant.objects.filter(activity_id=pk, name=saveTemp['name'])) == 1:
                participant_new = Participant.objects.get(
                    activity_id=pk, name=saveTemp['name'])
                participant_old.delete()
                ProcessHasParticipant.objects.create(
                    participant_id=participant_new.id, process_id=process.id)
            ...
            if len(Participant.objects.filter(activity_id=pk, name=saveTemp['name'])) == 0:
                ...
                participant_new = Participant.objects.get(
                    activity_id=pk, name=saveTemp['name'])
                processHasParticipant_old = ProcessHasParticipant.objects.get(
                    process_id=process.id, participant_id=participant_old.id)
                processHasParticipant_old.delete()
                ProcessHasParticipant.objects.create(
                    process_id=process.id, participant_id=participant_new.id)
        ...
        if len(ProcessHasSystem.objects.filter(system_id=system_old.id)) == 1:  # 舊的name只有一個物件
            ...
            # 新的name已有物件
            elif len(System.objects.filter(activity_id=pk, name=saveTemp['name'])) == 1:
                system_new = System.objects.get(
                    activity_id=pk, name=saveTemp['name'])
                system_old.delete()
                ProcessHasSystem.objects.create(
                    system_id=system_new.id, process_id=process.id)
            ...
            if len(System.objects.filter(activity_id=pk, name=saveTemp['name'])) == 0:
                ...
                system_new = System.objects.get(
                    activity_id=pk, name=saveTemp['name'])
                processHasSystem_old = ProcessHasSystem.objects.get(
                    process_id=process.id, system_id=system_old.id)
                processHasSystem_old.delete()
                ProcessHasSystem.objects.create(
                    process_id=process.id, system_id=system_new.id)
        ...
        if len(ProcessHasPii.objects.filter(pii_id=pii_old.id)) == 1:  # 舊的name只有一個物件
            ...
            # 新的name已有物件
            elif len(Pii.objects.filter(activity_id=pk, name=saveTemp['name'])) == 1:
                pii_new = Pii.objects.get(
                    activity_id=pk, name=saveTemp['name'])
                pii_old.delete()
                ProcessHasPii.objects.create(
                    pii_id=pii_new.id, process_id=process.id)
            ...
            if len(Pii.objects.filter(activity_id=pk, name=saveTemp['name'])) == 0:
                ...
                pii_new = Pii.objects.get(
                    activity_id=pk, name=saveTemp['name'])
                processHasPii_old = ProcessHasPii.objects.get(
                    process_id=process.id, pii_id=pii_old.id)
                processHasPii_old.delete()
                ProcessHasPii.objects.create(
                    process_id=process.id, pii_id=pii_new.id)
        ...
        process_old = Process.objects.get(
            activity_id=pk, name=saveTemp['old_name'])
                ...
                for process_has_pii in ProcessHasPii.objects.filter(process_id=process.id):
                    ...
                    for process_has_system in ProcessHasSystem.objects.filter(process_id=process.id):
                        system = System.objects.get(
                            id=process_has_system.system_id)
                    ...
                    for process_has_participant in ProcessHasParticipant.objects.filter(process_id=process.id):
                        participant = Participant.objects.get(
                            id=process_has_participant.participant_id)
                        evaluation = Evaluation.objects.create(
